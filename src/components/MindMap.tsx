...
const MindMap: React.FC<MindMapProps> = ({ data, onCoordinatesUpdate }) => {
      ...
      for (const song of data) {
        try {
          ...
          if (cover) {
            ...
            console.log(
              `No cover found for ${song.song_name} - ${song.artist}`
            );
          }
        } catch (error) {
  ...
  useEffect(() => {
    ...
    const drag = d3
      .drag<SVGGElement, SongData>()
      .on("start", (event) => {
        if (event.sourceEvent) {
          event.sourceEvent.stopPropagation();
        }
        d3.select(event.sourceEvent?.target || event.target).classed(
          "dragging",
          true
        );
      })
      .on("drag", (event, d) => {
        ...
        node.attr(
          "transform",
          `translate(${x * currentTransform.k + currentTransform.x}, ${
            y * currentTransform.k + currentTransform.y
          })`
        );
        ...
        if (onCoordinatesUpdate) {
          onCoordinatesUpdate(d.song_name, d.artist, {
            x: normalizedX,
            y: normalizedY,
            z: d.coordinates2.z,
          });
    ...
    nodes.each(function (this: SVGGElement, d: SongData) {
      ...
      root.render(
        <SongIcon
          albumArtUrl={
            albumCovers[`${d.song_name}-${d.artist}`] ||
            "data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjAwIiBoZWlnaHQ9IjIwMCIgdmlld0JveD0iMCAwIDIwMCAyMDAiIGZpbGw9Im5vbmUiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PHJlY3Qgd2lkdGg9IjIwMCIgaGVpZ2h0PSIyMDAiIGZpbGw9ImJsYWNrIi8+PC9zdmc+"
          }
          title={d.song_name}
          artist={d.artist}
    ...
    // Add tooltips
    nodes.append("title").text((d: SongData) => `${d.song_name} - ${d.artist}`);
  }, [
    localData,
    albumCovers,
    onCoordinatesUpdate,
